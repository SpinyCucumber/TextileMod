...
public class SpinningWheelRenderer extends TileEntityRenderer<SpinningWheelTile> {
	...
	public void render(SpinningWheelTile tileEntityIn, float partialTicks, MatrixStack matrixStackIn,
		...
		if (tileEntityIn.hasThread()) {
			...
			Color4f color = new Color4f(threadColor, threadAmount / (float) SpinningWheelTile.REQUIRED_THREAD);
			...
			// Render model
			CuboidRenderer.INSTANCE.renderCube(threadModel, matrixStackIn, buffer, color, combinedLightIn);
			// Undo rotation
			matrixStackIn.pop();
		}
	}
