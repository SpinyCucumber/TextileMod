...
public class CuboidRenderer {
	...
	public void renderCube(CuboidModel cube, MatrixStack matrix, IVertexBuilder buffer, Color4f color, int light, int overlay) {
		...
		for (Direction face : Direction.values()) {
			if (cube.shouldSideRender(face)) {
				...
				if (sprite != null) {
					// Get the two perpendicular axes to the face
					Axis u = face.getAxis() == Axis.X ? Axis.Z : Axis.X;
					Axis v = face.getAxis() == Axis.Y ? Axis.Z : Axis.Y;
					// Get how far along the face is on its normal axis
					...
					// Get the length of the cube along the two perpendicular axes
	...
	private void renderPoint(Matrix4f matrix4f, IVertexBuilder buffer, Direction face, Axis u, Axis v, float other,
			float[] uv, float[] xyz, boolean minU, boolean minV, float red, float green, float blue, float alpha,
			int light, int overlay) {
		int uIndex = minU ? U_MIN : U_MAX;
		int vIndex = minV ? V_MIN : V_MAX;
		Vector3f vertex = withValue(VEC_ZERO, u, xyz[uIndex]);
		vertex = withValue(vertex, v, xyz[vIndex]);
		vertex = withValue(vertex, face.getAxis(), other);
		Vector3f normal = face.toVector3f();
		buffer.pos(matrix4f, vertex.getX(), vertex.getY(), vertex.getZ()).color(red, green, blue, alpha)
				.tex(uv[uIndex], uv[vIndex]).overlay(overlay).lightmap(light).normal(normal.getX(), normal.getY(), normal.getZ()).endVertex();
	}
}