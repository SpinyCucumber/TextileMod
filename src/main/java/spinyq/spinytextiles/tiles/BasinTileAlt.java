...
import spinyq.spinytextiles.utility.NBTHelper.ClassSpace;
...
public class BasinTileAlt extends TileEntity {
	...
	private static final ClassSpace CLASS_SPACE = new ClassSpace(EmptyState.class, FilledState.class,
	...
	public class FilledState implements IBasinState {
		 ...
		 *
		 */
		public class DyeState implements IConsumerState {

			private RYBKColor color = new RYBKColor();

			private IDyeProvider provider = new IDyeProvider() {
		 ...
		 *
		 */
		public class BleachState implements IConsumerState {

			private float bleachLevel = 0.0f;

			private IBleachProvider provider = new IBleachProvider() {
		...
		private IBasinState subState;
		private ClassSpace.Mapper mapper = CLASS_SPACE.createMapper()
				.with(WaterState.class, WaterState::new)
				.with(DyeState.class, DyeState::new)
				.with(BleachState.class, BleachState::new);
	...
	private IBasinState state;
	private ClassSpace.Mapper mapper = CLASS_SPACE.createMapper().with(EmptyState.class, EmptyState::new)
			.with(FilledState.class, FilledState::new);
